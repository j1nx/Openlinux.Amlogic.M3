...
//SONY_TRACE_ENTER("sony_stopwatch_start");
   ...
   //unsigned long delay = jiffies + 5000*100; //5*HZ = 5̑
    ///while(delay > jiffies);
	 printk("sony_stopwatch_start pStopwatch->jiffies %ul\n",pStopwatch->startTime);
   printk("sony_stopwatch_start -------\n");
...
//#error sony_stopwatch_elapsed is not implemented
    ...
    printk("sony_stopwatch_start pStopwatch->jiffies %ul,*pElapsed %d-------\n",jiffies ,*pElapsed);
	   SONY_TRACE_RETURN(SONY_RESULT_OK);
