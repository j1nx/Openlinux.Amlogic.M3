...
typedef struct
{
  ...
  DMD_u32_t ifOutFreqinKHz;                 /* IN, IF Frequency in KHz */
  UINT8 ifOutGainLevel;                  /* IF out gain level */ 
} MXL603_TUNER_MODE_CFG_T;
...
typedef struct
{
  DMD_u32_t freqInHz;                     /* IN, Radio Frequency in Hz */ 
...
typedef struct
{
  ...
  DMD_u32_t manualIFOutFreqInKHz;      /* IN, IF out frequency selection when manualFreqSet = 0*/
}MXL603_IF_OUT_CFG_T;
...
//separate MXL603 init functions to several ones
extern MXL_STATUS MXL603_INIT(void);
extern MXL_STATUS Mxl603SetSystemMode(void);
extern MXL_STATUS Mxl603SetFreqBw(void);
