...
static const UINT8 MxLWare603DrvVersion[] = {2, 1, 1, 3, 0}; 
                                           ...
                                           MXL603_CHAN_TUNE_CFG_T chanTuneCfg)
{
  ...
  chanTuneCfg.freqInHz=chanTuneCfg.freqInHz/1000;  //by rsj
  ...
  if (chanTuneCfg.startTune == MXL_ENABLE)
  {
   ...
   //for test rsj
   		UINT8 DataPtr;
   		MxLWare603_OEM_ReadRegister(devId, 0x7C, &DataPtr);
		printk("[7C] is %x\n",DataPtr);
      }
      else 
      {
      ...
      switch(chanTuneCfg.bandWidth)
      {
        ...
        case MXL603_TERR_BW_8MHz:
            ...
            frequency *= 64;
           // freq = (DMD_u32_t)(frequency / 1000000); 
           freq = (frequency / 1000); //by rsj
		   printk("freq is %d\n",freq);
            // Set RF  
