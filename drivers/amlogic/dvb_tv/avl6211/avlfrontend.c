...
static int AVL6211_Lnb_Power_Ctrl(int lnb_power_gpio, int lnb)
{
	if(1==lnb)	gpio_direction_output(lnb_power_gpio, 1);
	else	gpio_direction_output(lnb_power_gpio, 0);
	return 0;
}

static int AVL6211_Tuner_Power_Ctrl(int tuner_power_gpio,int tunerpwr)
{
	if(1==tunerpwr)	gpio_direction_output(tuner_power_gpio, 1);
	else	gpio_direction_output(tuner_power_gpio, 0);
	return 0;
}

static int AVL6211_Ant_Overload_Ctrl(int antoverload_gpio)
{
	return gpio_get_value(antoverload_gpio);
}
...
//	AVL6211_Lnb_Power_Ctrl(1);
	//tunerpower
	AVL6211_Tuner_Power_Ctrl(dev->tuner_power_gpio, 0);
	//init
