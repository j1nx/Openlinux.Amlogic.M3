...
#if 0//defined(CONFIG_ARCH_MESON2)||(MESON_CPU_TYPE == MESON_CPU_TYPE_MESON6TV)
...
static int force_update_post_reg = 2; /* set force_update_post_reg = 1: only update post reg in vsync for one time 
                                         set force_update_post_reg = 2: always update post reg in vsync
                                      */
static int input2pre_mask = 0;
...
/* 
trans_fmt:
bit[7:0]:  HDMI 3D type
bit[15:8]:  detected 3D type
*/
static unsigned int trans_fmt = 0; 

int force_duration_0 = 0;

#ifdef NEW_DI 
static int di_new_mode_enable = 1;
...
static unsigned int force_recovery=0;
...
/**/
#define real_buf_mgr_mode ((buf_mgr_mode&buf_mgr_mode_mask)|(buf_mgr_mode_mask>>16))
...
static unsigned char is_bypass(void)
{
    ...
    if(di_pre_stru.cur_prog_flag&&
        ...
        return 1;	
    if(bypass_hd&&
        ((di_pre_stru.cur_width>=1280)||(di_pre_stru.cur_height>=720))
      )
      return 1;
...
static void dump_state(void)
{
    ...
    int itmp;
    int i;
    printk("provider vframe level %d, init_flag %d, is_bypass %d, buf_mgr_mode 0x%x buf_mgr_mode_mask 0x%x\n", 
        provider_vframe_level, init_flag, is_bypass(), buf_mgr_mode, buf_mgr_mode_mask);
...
//        printk("index %d, %x, type %d, vframetype 0x%x, trans_fmt %u\n", p->index, (unsigned int)p, p->type,p->vframe->type,p->vframe->trans_fmt);
    }
...
//        printk("index %d, 0x%x, type %d, vframetype 0x%x, trans_fmt %u\n", p->index, (unsigned int)p, p->type ,p->vframe->type,p->vframe->trans_fmt);
    ...
    queue_for_each_entry(p, ptmp, QUEUE_POST_READY, list) {
//        printk("index %d, %x, type %d, vframetype 0x%x, trans_fmt %u,duration %d pts %d\n", p->index, (unsigned int)p, p->type, p->vframe->type, 
//                        p->vframe->trans_fmt,p->vframe->duration, p->vframe->pts);
        if(p->di_buf[0]){
//            printk("    +index %d, 0x%x, type %d, vframetype 0x%x, trans_fmt %u\n", p->di_buf[0]->index, (unsigned int)p->di_buf[0], p->di_buf[0]->type, 
 //                       p->di_buf[0]->vframe->type,p->di_buf[0]->vframe->trans_fmt);
        }
        if(p->di_buf[1]){
//            printk("    +index %d, 0x%x, type %d, vframetype 0x%x, trans_fmt %u\n", p->di_buf[1]->index, (unsigned int)p->di_buf[1], p->di_buf[1]->type, 
//                        p->di_buf[1]->vframe->type,p->di_buf[1]->vframe->trans_fmt);
        }
    }
    printk("display_list:\n");
    queue_for_each_entry(p, ptmp, QUEUE_DISPLAY, list) {
        printk("index %d, 0x%x, type %d, vframetype 0x%x duration %d pts %d\n", p->index, (unsigned int)p, p->type, p->vframe->type, p->vframe->duration, p->vframe->pts);
        if(p->di_buf[0]){
//            printk("    +index %d, 0x%x, type %d, vframetype 0x%x, trans_fmt %u\n", p->di_buf[0]->index, (unsigned int)p->di_buf[0], p->di_buf[0]->type, 
//                        p->di_buf[0]->vframe->type,p->di_buf[0]->vframe->trans_fmt);
        }
...
#define WR_CANVAS_WID                   8
        ...
        trans_fmt = (di_pre_stru.det3d_trans_fmt << 8 )|di_pre_stru.source_trans_fmt;
#endif
...
static int do_pre_only_fun(void* arg)
{
    ...
    if(arg){
        ...
        canvas_config(DI_POST_BUF0_CANVAS_IDX, di_buf->di_buf[0]->nr_adr, width*2, canvas_height, 0, 0);
                    
        vf->canvas0Addr = DI_POST_BUF0_CANVAS_IDX;
        vf->canvas1Addr = DI_POST_BUF0_CANVAS_IDX;
    }
#endif
     ...
     unsigned zoom_end_x_lines, unsigned zoom_start_y_lines, unsigned zoom_end_y_lines)
{
    ...
    if((di_post_stru.post_process_fun_index != 1)||(force_update_post_reg!=0)){
...
//printk("height = (%d %d %d %d %d)\n", di_buf->vframe->height, zoom_start_x_lines, zoom_end_x_lines, zoom_start_y_lines, zoom_end_y_lines);
    ...
    if(di_post_stru.update_post_reg_flag)
	  ...
	  di_post_stru.update_post_reg_flag = 0;
    return 0;
}
     ...
     unsigned zoom_end_x_lines, unsigned zoom_start_y_lines, unsigned zoom_end_y_lines)
{
    ...
    if((di_post_stru.post_process_fun_index != 2)||(force_update_post_reg!=0)){
        force_update_post_reg &= ~0x1;
     ...
     unsigned zoom_end_x_lines, unsigned zoom_start_y_lines, unsigned zoom_end_y_lines)
{
    ...
    if((di_post_stru.post_process_fun_index != 3)||(force_update_post_reg!=0)){
        force_update_post_reg &= ~0x1;
...
#ifdef DET3D
            if( di_pre_stru.det3d_trans_fmt != 0){
                if(ready_di_buf->vframe->trans_fmt == 0 || det3d_en){
...
#endif
            if(is_progressive(vframe)&&is_from_vdin(vframe)&&(prog_proc_config&0x1)){
#if !(defined RUN_DI_PROCESS_IN_IRQ)
                spin_lock_irqsave(&plist_lock, flags);
#endif
                raw_local_save_flags(fiq_flag);
...
get_vframe:
    ...
    if(di_post_stru.run_early_proc_fun_flag && vframe_ret){
        if(vframe_ret->early_process_fun){
...
MODULE_PARM_DESC(trans_fmt, "\n trans_fmt\n");
module_param(trans_fmt, uint, 0664);


#ifdef NEW_DI
MODULE_PARM_DESC(di_new_mode_enable, "\n di_new_mode_enable\n");
module_param(di_new_mode_enable, uint, 0664);
#endif
